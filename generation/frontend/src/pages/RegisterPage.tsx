...
export function RegisterPage() {
  ...
  async function createAccount(e: FormEvent) {
    e.preventDefault(); setError(null); setSubmitting(true);
    try {
      await register(phone, password, displayName, username, dateOfBirth, avatarUrl, bio, interests.split(',').map((x) => x.trim()).filter(Boolean));
      navigate('/home');
    } catch (err) { setError(apiErrorMessage(err)); } finally { setSubmitting(false); }
  }
  ...
  return <div className="flex min-h-screen items-center justify-center bg-paper px-6"><div className="w-full max-w-sm">
    ...
    {step === 'profile' && <form onSubmit={createAccount}><h1 className="mt-8 font-display text-2xl text-navy">Set up your profile</h1><input className={field} required placeholder="Display name" value={displayName} onChange={(e) => setDisplayName(e.target.value)} /><div className="relative"><span className="absolute left-3 top-3 text-sm text-ink/50">@</span><input className={`${field} pl-7`} required minLength={6} maxLength={20} pattern="[a-z0-9_]+" placeholder="username" value={username} onChange={(e) => setUsername(e.target.value.toLowerCase())} /></div><p className="-mt-3 text-xs text-ink/45">6–20 lowercase letters, numbers, or underscores. This is public.</p><input className={field} type="password" required minLength={8} placeholder="Development password" value={password} onChange={(e) => setPassword(e.target.value)} /><input className={field} type="url" placeholder="Profile photo URL (optional)" value={avatarUrl} onChange={(e) => setAvatarUrl(e.target.value)} /><textarea className={field} placeholder="Bio (optional)" value={bio} onChange={(e) => setBio(e.target.value)} /><input className={field} placeholder="Interests, separated by commas (optional)" value={interests} onChange={(e) => setInterests(e.target.value)} />{error && <p className="mt-3 text-sm text-red-600">{error}</p>}<button disabled={submitting} className={button}>Enter Generation</button></form>}
